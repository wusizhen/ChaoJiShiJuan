...
{
	 	//tbSingleDAL
		public partial class tbSingleDAL
	{
   		     
		public bool Exists(int id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from tbSingle");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.tbSingle model)
		{
			StringBuilder strSql=new StringBuilder();
...
SqlParameter[] parameters = {
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tbSingle ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.tbSingle GetModel(int id)
		{
		...
		public DataSet GetListByIndex(int pageSize,int pageIndex,string strWhere,string filedOrder)
		{
            ...
            coreSql.Append("select top " + (pageIndex - 1) * pageSize + " id from tbSingle where 1=1 ");
            if (strWhere.Trim() != "")
            {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + pageSize + " * from tbSingle where 1=1 ");
            if (strWhere.Trim() != "")
            {
		...
		public int GetCount()
		{
			String strSql="select count(*) from tbSingle";
			Util.MyUtil.PrintSql(strSql.ToString());
		...
		{
			String strSql="select count(*) from tbSingle where "+strWhere;
			Util.MyUtil.PrintSql(strSql.ToString());
			return Convert.ToInt32(DbHelperSQL.GetSingle(strSql));
		}
		...
		/// 更新一条数据
		/// </summary>
		public bool UpdateTran(Model.tbSingle model,SqlTransaction transaction)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tbSingle set ");
