...
{
	 	//tbJudgeDAL
		public partial class tbJudgeDAL
	{
   		     
		public bool Exists(int id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from tbJudge");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tbJudge(");			
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tbJudge set ");
...
SqlParameter[] parameters = {
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tbJudge ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
		...
		public Model.tbJudge GetModel(int id)
		{
			...
			parameters[0].Value = id;
			 ...
			 Util.MyUtil.PrintSql(strSql.ToString());
			Model.tbJudge model=new Model.tbJudge();
		...
		public DataSet GetListByIndex(int pageSize,int pageIndex,string strWhere,string filedOrder)
		{
            ...
            coreSql.Append("select top " + (pageIndex - 1) * pageSize + " id from tbJudge where 1=1 ");
            if (strWhere.Trim() != "")
            {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + pageSize + " * from tbJudge where 1=1 ");
            if (strWhere.Trim() != "")
            {
		...
		public int GetCount()
		{
			String strSql="select count(*) from tbJudge";
			Util.MyUtil.PrintSql(strSql.ToString());
		...
		{
			String strSql="select count(*) from tbJudge where "+strWhere;
			Util.MyUtil.PrintSql(strSql.ToString());
			return Convert.ToInt32(DbHelperSQL.GetSingle(strSql));
		}
		...
		/// 增加一条数据
		/// </summary>
		public int AddTran(Model.tbJudge model,SqlTransaction transaction)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tbJudge(");			
            strSql.Append("chapterid,ques,ans,diff,selectcount,rightcount,questype");
		...
		/// 更新一条数据
		/// </summary>
		public bool UpdateTran(Model.tbJudge model,SqlTransaction transaction)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tbJudge set ");
...
SqlParameter[] parameters = {
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.tbJudge GetModelTran(int id ,SqlTransaction transaction)
		{
